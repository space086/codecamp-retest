...
import MarketsDetailUI from './MarketsDetail.presenter'
import { FETCH_USED_ITEM, DELETE_USED_ITEM } from './MarketsDetail.queries'
...
export default function MarketsDetail(){
  ...
  const onClickbaskets = () => {
    const baskets =JSON.parse(localStorage.getItem("baskets") || "[]")
      ...
      // 2. 이미 담겼는지 확인하기
      const temp = baskets.filter((basketEl) => basketEl._id === data?.fetchUseditem._id) // [{...}]
      if(temp.length === 1) {
        alert("이미 담으신 물품입니다.")
        return;
      }
  ...
  useEffect(() => {
    ...
    script.src = "//dapi.kakao.com/v2/maps/sdk.js?appkey=b22fcdd40598d9e0b81b04d7a2e012cd&autoload=false&libraries=services"
    ...
    script.onload = () => {
      window.kakao.maps.load(function() {
        ...
        // 주소로 좌표를 검색합니다
        geocoder.addressSearch(data?.fetchUseditem.useditemAddress.address !==""? data?.fetchUseditem.useditemAddress.address : '제주특별자치도 제주시 첨단로 242' , function(result, status) {
